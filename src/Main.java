...
public class Main extends JFrame {
    ...
    */
    ...
    public static String[][] nameAI = new String[][]{
        {"Gerald", "Günther", "Katrin", "Marius"},
        {"Jörg", "Manuela", "Stefan", "Jens"},
        {"Helmuth", "Detlef", "Gunnar", "Gabi"}
    };
    ...
    public static String[] NOM_MAHJONG_SPE = {"", "Les Pung Venteux", "Les paires venteuses",
        ...
        "Les 13 lanternes merveilleuses", "Les 7 muses du po�te chinois", "Les 4 bonheurs domestiques",
        "Le triangle �ternel", "La temp�te", "Le souffle du dragon", "La petite main verte",
        ...
        "La main blanche et verte", "Le mandarin", "Le mahjong imp�rial"};
    ...
    JMenu fich = new JMenu(messages.getString("file"));
    JMenu info = new JMenu(messages.getString("help"));
    JMenuItem nouv = new JMenuItem(messages.getString("new"));
    JMenuItem option = new JMenuItem(messages.getString("options"));
    JMenuItem jHighScore = new JMenuItem(messages.getString("highscore"));
    JMenuItem quit = new JMenuItem(messages.getString("leave"));
    JMenuItem regle = new JMenuItem(messages.getString("rules"));
    JMenuItem aPropos = new JMenuItem(messages.getString("about"));

    /* Boutons*/
    JButton jouer = new JButton(messages.getString("play"));
    JButton declar = new JButton(messages.getString("report"));
    JButton prendre = new JButton(messages.getString("have"));
        ...
        ActionListener actionTimer = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                if (timerCpt == 0 && timer.isRunning()) {
                    timer.stop();
                    // v�rifie si un des ordi peut prendre la tuile du milieu pour faire un Mahjong
                    for (int i = 1; i < 4; i++) {
                        if (joueurs[i].peutMahjong(discard)) {
                            joueurs[i].ajouteTuile(discard);	//r�cup�re la tuile
                    ...
                    for (int i = 1; i < 4; i++) { 	 //un ordi peut-il prendre la tuile pour faire une mainExpose ? 
                        if (joueurs[i].peutPrendre(discard) && aQuiDeJouer != i) {  //ce joueur peut-il prendre ? 
                            ...
                            joueurs[aQuiDeJouer].labelVent.setForeground(new Color(0, 0, 0));	//remet en noir le joueur pr�cedent
            ...
            for (int i = 0; i < 14; i++) {
                ...
                if ((x > (X + i * 37)) && (x < (X + (i + 1) * 37)) && (y > (Y + 50)) && (y < (Y + 99))) {
                    if (joueurs[0].mainCache.estVide(i) == false) {	// test si la tuile i existe
                        if (temp < 0) {	// aucune tuile n'avait �t� s�l�ction� avant 
                            joueurs[0].labelCache[i].setBounds(X + 37 * i, Y - 10, 37, 49);	//d�calage vers le haut
                            joueurs[0].tuileSelect = i;									//m�morisation de la tuile
                            ...
                            if (joueurs[0].tuileSelect != i) {	//la tuile s�l�ction�e n'est pas la m�me qu'avant
                                joueurs[0].labelCache[i].setBounds(X + 37 * i, Y - 10, 37, 49);	//d�calage vers le haut
                                joueurs[0].tuileSelect = i;									//m�morisation de la tuile
                            } else {								//la m�me tuile est s�l�ction�e
                                joueurs[0].tuileSelect = -1;		//aucune tuile n'est s�l�ction�e
                            }
                        }
                    }
                    break;
    ...
    void nouveauJeu() {
        ...
        jCB1 = new JComboBox(nameAI[0]);
        jCB2 = new JComboBox(nameAI[1]);
        jCB3 = new JComboBox(nameAI[2]);
        ...
        //maj des type d'IA
        joueurs[1].nom = nameAI[0][jCB1.getSelectedIndex()];
        joueurs[2].nom = nameAI[1][jCB2.getSelectedIndex()];
        joueurs[3].nom = nameAI[2][jCB3.getSelectedIndex()];
        ...
        if (aQuiDeJouer == 0) { // c'est au joueur de jouer
            if (joueurs[0].aKong() >= 0 || joueurs[0].aMahjong()) {
                declar.setEnabled(true);	//le joueur peut d�clarer une combi ou un mahjong
            } else {
                declar.setEnabled(false);
            }
            jouer.setEnabled(true);
     ...
     *
     * @param estGagne : vrai si le vent d'est a gagn� (les vents ne tournent
     * pas)
     */
...
//			else
    ...
    void jouer() {
        ...
        if (timer.isRunning()) {	// le timer tourne dc le joueur a demand� de passer au joueur suivant 
            ...
            // v�rifie si les ordi peuvent prendre la tuile jet�e pour faire mahjong
            for (int i = 1; i < 4; i++) {
                if (joueurs[i].peutMahjong(discard)) {
                    joueurs[i].ajouteTuile(discard);
                    joueurs[i].declareFigure(discard, false);
            ...
            // v�rifie si les ordi peuvent prendre la tuile jet�e pour faire une combi
            for (int i = 1; i < 4; i++) {
                if (joueurs[i].peutPrendre(discard) && aQuiDeJouer != i) { // ce joueur peut prendre ?
                    ...
                    aQuiDeJouer = i;
                    if (cpt == 4) {// si le joueur d�clare un kong, il doit piocher
                        partie(true);
                    } else {
                        partie(false);
                    }
            ...
            if (joueurs[0].tuileSelect != -1) {	//il faut avoir s�l�ctionner une tuile pour jouer
    ...
    void partie(boolean doitPiocher) {
        ...
        if (doitPiocher == true) {
            if ((joueurs[aQuiDeJouer].mainCache.getNbTuile() % 3) != 1) {
                Object[] obj = {"BUG: " + joueurs[aQuiDeJouer].nom + " a " + joueurs[aQuiDeJouer].mainCache.getNbTuile() + " tuiles cach�e (prb1).\nPr�venir Raf :-D"};
            ...
            if ((joueurs[aQuiDeJouer].mainCache.getNbTuile() % 3) != 2) {
                Object[] obj = {"BUG: " + joueurs[aQuiDeJouer].nom + " a " + joueurs[aQuiDeJouer].mainCache.getNbTuile() + " tuiles cach�e (prb2).\nPr�venir Raf :-D"};
                JOptionPane.showMessageDialog(null, obj, "Bug Report", JOptionPane.ERROR_MESSAGE);
            }
        }
        ...
        if (indexPioche >= NB_TUILES) { // plus de tuile
            ...
            if (aQuiDeJouer == 0) {
                if (temp2 >= 0) {	// si le joueur a pioch� une tuile, on la sureleve
                    joueurs[0].labelCache[temp2].setBounds(X + 37 * temp2, Y - 10, 37, 49);	//d�calage vers le haut
                ...
                //pour debug: detecte incoherence du nb de tuile dans un jeu
                if ((joueurs[aQuiDeJouer].mainCache.getNbTuile() % 3) != 2) {
                    Object[] obj = {"BUG: " + joueurs[aQuiDeJouer].nom + " a " + joueurs[aQuiDeJouer].mainCache.getNbTuile() + " tuiles cach�e (prb3).\nPr�venir Raf :-D"};
                    JOptionPane.showMessageDialog(null, obj, "Bug Report", JOptionPane.ERROR_MESSAGE);
                }
                ...
                if (joueurs[aQuiDeJouer].aMahjong()) {
                    ...
                    while (nb >= 0) {
                        System.out.print(joueurs[aQuiDeJouer].nom + " d�clare un kong de " + joueurs[aQuiDeJouer].mainCache.figures[nb].nom() + "\n");
                    ...
                    //pour debug: detecte incoherence du nb de tuile dans un jeu
                    if ((joueurs[aQuiDeJouer].mainCache.getNbTuile() % 3) != 1) {
                        Object[] obj = {"BUG: " + joueurs[aQuiDeJouer].nom + " a " + joueurs[aQuiDeJouer].mainCache.getNbTuile() + " tuiles cach�e (prb4).\nPr�venir Raf :-D"};
                        JOptionPane.showMessageDialog(null, obj, "Bug Report", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
    ...
    int IAChoixTuile(Joueur j) {
        ...
        switch (j.typeIA) {
            case 0:
            default:
                do { // on jete une tuile au hasard sauf si l'ordi poss�de plus d'1 exemplaire 
            ...
            case 1:
                ...
                if (jeuTemp.getFirstFig(3, DESCENDANT) >= 0) {		//s'il y a des tuiles jet�es plus de 3 fois
                ...
                } else { // il n'y a que des tuiles qui ont �t� j�t� une fois ou jamais
                    do { // on jete une tuile au hasard sauf si l'ordi poss�de plus d'1 exemplaire 
                        figChoisie = rand.nextInt(14);
                        if (cpt++ > 14 && j.mainCache.figures[figChoisie].estLibre() == false) {
                            break;
            ...
            case 2:
                ...
                } else if (jeuTemp.getFirstFig(2, DESCENDANT) >= 0) {	//s'il y a des tuiles jet�es plus de 2 fois
    ...
    void calculScore(int gagnant) {
                    ...
                    if (i == gagnant) {		// je suis gagnant
                        ...
                        if (j == gagnant) {	// l'autre a gagn�
                            if (scoreInter[i] > scoreInter[j] && joueurs[j].vent == EST) {	//j'ai un score plus grand et le gagnant est EST
                                scoreFinal[i] += 4 * (scoreInter[i] - scoreInter[j]);	// je re�ois 4* la diff de nos score
                            } else {
                                if (joueurs[i].vent == EST || joueurs[j].vent == EST) {	// si je suis EST ou que l'aute est EST
                            ...
                            if (joueurs[i].vent == EST) {	// si je suis EST 
                                ...
                                scoreFinal[i] += scoreInter[i] - scoreInter[j];	// sinon diff�rence normale
                            }
                        }
                    }
                } else {
        ...
        if (gagnant == 0) {
            ...
            if (isBetterScore) {
                ...
                String message = "Bien jou�! Vous �tes class� dans les high scores (" + scoreFinal[0] + " pts)";
