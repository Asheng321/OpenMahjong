...
public class Main extends JFrame {
	...
	/*Scores des Mahjong spéciaux (idx0 ne compte pas)
	 * 
	 */
	...
	public static String[] NOM_MAHJONG_SPE = {"", "Les Pung Venteux", "Les paires venteuses",
		...
		"Les 13 lanternes merveilleuses", "Les 7 muses du poète chinois", "Les 4 bonheurs domestiques",
		"Le triangle éternel", "La tempête", "Le souffle du dragon", "La petite main verte", 
		...
		"La main blanche et verte", "Le mandarin", "Le mahjong impérial"};
	...
	JButton declar = new JButton("Déclarer");
	...
	/** Crée un nouvelle espace graphique */
	public Main() {
		/* Propriété de Base */
		...
		ActionListener actionTimer = new ActionListener (){
			...
			public void actionPerformed (ActionEvent event)
			{
				...
				if(timerCpt==0 && timer.isRunning()){
					timer.stop();
					// vérifie si un des ordi peut prendre la tuile du milieu pour faire un Mahjong
					for(int i=1; i<4; i++){
						if(joueurs[i].peutMahjong(discard)){
							joueurs[i].ajouteTuile(discard);	//récupère la tuile
					...
					for(int i=1; i<4; i++){ 	 //un ordi peut-il prendre la tuile pour faire une mainExpose ? 
						if(joueurs[i].peutPrendre(discard) && aQuiDeJouer!=i){  //ce joueur peut-il prendre ? 
							...
							joueurs[i].declareFigure(discard,false);
							poubelle.takeDiscardedTile();
							poubelle.declare(discard, cpt-1); //ajoute les tuiles à la poubelle

							joueurs[aQuiDeJouer].labelVent.setForeground(new Color(0,0,0));	//remet en noir le joueur précedent
							aQuiDeJouer = i;	//donne la mainCache au joueur qui prend
							
							poubelle.affiche(joueurs[0].mainCache);
							if(cpt==4){
								//displayText(joueurs[i].nom+"("+joueurs[i].numero+") déclare un kong");
								partie(true);
							}
							else{
								//displayText(joueurs[i].nom+"("+joueurs[i].numero+") déclare un pung");
								partie(false);
							}
							return;		 //retour car ce prog est fait n'importe comment... 
						}
					}
					// personne ne prendre la tuile jetée
		...
		for (int i=0; i<14; i++){	// première ligne de combi
		...
		for (int i=14; i<24; i++){	// deuxième ligne
			joueurs[0].labelExpose[i] = new JLabel();
			joueurs[1].labelExpose[i] = new JLabel();
			joueurs[2].labelExpose[i] = new JLabel();
			joueurs[3].labelExpose[i] = new JLabel();
			...
			for (int i=0; i<14; i++){
				...
				if ((x>(X+i*37))&&(x<(X+(i+1)*37))&&(y>(Y+50))&&(y<(Y+99))){
					if(joueurs[0].mainCache.estVide(i) == false) {	// test si la tuile i existe
						if(temp < 0){	// aucune tuile n'avait été séléctioné avant 
							joueurs[0].labelCache[i].setBounds(X+37*i, Y-10, 37, 49);	//décalage vers le haut
							joueurs[0].tuileSelect = i;									//mémorisation de la tuile
						}
						else{	//une tuile avait déjà été séléctionée
							joueurs[0].labelCache[temp].setBounds(X+37*temp, Y, 37, 49);	//on remet la tuile d'avant en place

							if(joueurs[0].tuileSelect != i){	//la tuile séléctionée n'est pas la même qu'avant
								joueurs[0].labelCache[i].setBounds(X+37*i, Y-10, 37, 49);	//décalage vers le haut
								joueurs[0].tuileSelect = i;									//mémorisation de la tuile
							}
							else{								//la même tuile est séléctionée
								joueurs[0].tuileSelect = -1;		//aucune tuile n'est séléctionée
							}
						}
					}
					break;
	...
	/**
	 * Gère l'affichage et la mise en oeuvre des options de jeu
	 */
	void options(){
		Checkbox montreJeux = new Checkbox("Montrer tous les jeux");
		Checkbox montreDisc = new Checkbox("Montrer les tuiles jetées");

		// on arrête le timer pendant le réglage des options
		if(timerCpt>0){
			timer.stop();
		}
		...
		//on redémarre le timer si necessaire
		if(timerCpt>0){
			timer.start();
		}
	...
	/**
	 * Crée un nouveau jeu (les scores sont remis à 0)
	 */
	void nouveauJeu(){
		boolean temp;
		...
		if(aQuiDeJouer == 0){ // c'est au joueur de jouer
			if(joueurs[0].aKong()>=0 || joueurs[0].aMahjong()){
				declar.setEnabled(true);	//le joueur peut déclarer une combi ou un mahjong
			}
			else{
				declar.setEnabled(false);
			}
	...
	/**
	 * Crée une nouvelle partie (score non remis à 0)
	 * @param estGagne : vrai si le vent d'est a gagné (les vents ne tournent pas)
	 */
	void nouvellePartie(boolean estGagne){
		...
		boolean temp = false;
		//chaque joueur déclare les fleurs et les saisons
		for(int i=0; i<4; i++){
			do{
				temp = joueurs[i].declareHonneur();
				if(temp) piocheTuiles(joueurs[i]);
	...
	void initPioche(){
		...
		//inverse 300 fois 2 tuiles aléatoires
		for(int i=0; i<300; i++){
...
//			else
	...
	/**
	 * Le joueur a appuyé sur le bouton Déclarer
	 * Permet de dévlarer des kong caché ou un Mahjong 
	 */
	void declarer()
	{
		int reponse = -1, nb;
		...
		if(joueurs[0].aMahjong()){
			reponse=JOptionPane.showConfirmDialog(null, "Vous avez un Mahjong!\nVoulez vous le déclarer?","MAHJONG!!!",  JOptionPane.YES_NO_OPTION);
			if(reponse == 0){	// le joueur déclare un Mahjong
				finPartie(0);
				return;
			}
		}
		...
		joueurs[0].affiche();
		// vérifie si le joueur peut encore déclarer
		if(joueurs[0].aKong()>=0 || joueurs[0].aMahjong()){
	...
	/**
	 * Le joueur a appuyé sur le bouton jouer/passer
	 */
	void jouer(){
		Tuile discard = poubelle.getDiscard();
		if(timer.isRunning()){	// le timer tourne dc le joueur a demandé de passer au joueur suivant 
			timer.stop();
			jouer.setText("Jouer");
			jouer.setEnabled(true);
			prendre.setEnabled(false);

			// vérifie si les ordi peuvent prendre la tuile jetée pour faire mahjong
			for(int i=1; i<4; i++){
				if(joueurs[i].peutMahjong(discard)){
					joueurs[i].ajouteTuile(discard);
					joueurs[i].declareFigure(discard, false);
			...
			// vérifie si les ordi peuvent prendre la tuile jetée pour faire une combi
			for(int i=1; i<4; i++){
				if(joueurs[i].peutPrendre(discard) && aQuiDeJouer!=i){ // ce joueur peut prendre ?
					...
					aQuiDeJouer = i;
					if(cpt==4){// si le joueur déclare un kong, il doit piocher
						partie(true);
					}
					else{
						partie(false);
			...
			if(joueurs[0].tuileSelect != -1){	//il faut avoir séléctionner une tuile pour jouer
	...
	/**
	 * Le joueur a appuyé sur le bouton Prendre 
	 */
	void prendre(){
		int cpt = 0;
		Tuile discard = poubelle.getDiscard();
	...
	void partie(boolean doitPiocher){
		boolean temp;
		int temp2 = -1; //temp2 est la position de la dernière tuile piochée
		boolean declarKong;

		//pour debug: detecte incoherence du nb de tuile dans un jeu
		if(doitPiocher==true){
			if((joueurs[aQuiDeJouer].mainCache.getNbTuile()%3)!=1){
				Object[] obj = {"BUG: "+joueurs[aQuiDeJouer].nom+" a "+joueurs[aQuiDeJouer].mainCache.getNbTuile()+" tuiles cachée (prb1).\nPrévenir Raf :-D"};
				JOptionPane.showMessageDialog(null,obj,"Bug Report",JOptionPane.ERROR_MESSAGE);
			}
		}
		else{
			//le joueur n'a pas à piocher
			if((joueurs[aQuiDeJouer].mainCache.getNbTuile()%3)!=2){
				Object[] obj = {"BUG: "+joueurs[aQuiDeJouer].nom+" a "+joueurs[aQuiDeJouer].mainCache.getNbTuile()+" tuiles cachée (prb2).\nPrévenir Raf :-D"};
				JOptionPane.showMessageDialog(null,obj,"Bug Report",JOptionPane.ERROR_MESSAGE);
			}
		}
		...
		if(doitPiocher == true){
			...
			if(indexPioche<NB_TUILES){
				...
				// déclaration des fleurs et saisons et des nouveau kong
				do{
		...
		if(indexPioche>=NB_TUILES){ // plus de tuile
			//check si la dernière tuile à fait Mahjong
			...
			if(aQuiDeJouer == 0){
				if(temp2 >=0){	// si le joueur a pioché une tuile, on la sureleve
					joueurs[0].labelCache[temp2].setBounds(X+37*temp2, Y-10, 37, 49);	//décalage vers le haut
					joueurs[0].tuileSelect = temp2;									//mémorisation de la tuile
				}
				...
				//pour debug: detecte incoherence du nb de tuile dans un jeu
				if((joueurs[aQuiDeJouer].mainCache.getNbTuile()%3)!=2){
					Object[] obj = {"BUG: "+joueurs[aQuiDeJouer].nom+" a "+joueurs[aQuiDeJouer].mainCache.getNbTuile()+" tuiles cachée (prb3).\nPrévenir Raf :-D"};
					JOptionPane.showMessageDialog(null,obj,"Bug Report",JOptionPane.ERROR_MESSAGE);
				}
			...
			// les ordinateurs jouent
				...
				if(joueurs[aQuiDeJouer].aMahjong()){
					...
					//déclaration des kong cachés
					int nb = joueurs[aQuiDeJouer].aKong();
					while(nb>=0){
						System.out.print(joueurs[aQuiDeJouer].nom+" déclare un kong de "+joueurs[aQuiDeJouer].mainCache.figures[nb].nom()+"\n");
					...
					//pour debug: detecte incoherence du nb de tuile dans un jeu
					if((joueurs[aQuiDeJouer].mainCache.getNbTuile()%3)!=1){
						Object[] obj = {"BUG: "+joueurs[aQuiDeJouer].nom+" a "+joueurs[aQuiDeJouer].mainCache.getNbTuile()+" tuiles cachée (prb4).\nPrévenir Raf :-D"};
						JOptionPane.showMessageDialog(null,obj,"Bug Report",JOptionPane.ERROR_MESSAGE);
					}
				}
			}
	 ...
	 * gagnant : -1 si personne n'a gagné, numéro du joueur gagnant sinon
	 */
		...
		if(gagnant<0){ //pas de gagnant
			...
			if(gagnant == 0){
				if(joueurs[0].aMahjongSpecial()>0){
					Object[] obj = {"Vous venez de réaliser un Mahjong Spécial!\n      ----\""+NOM_MAHJONG_SPE[joueurs[0].aMahjongSpecial()]+"\"----\n BIEN JOUER!"};
					JOptionPane.showMessageDialog(null,obj,"Fin de la partie",JOptionPane.INFORMATION_MESSAGE);					
				}
				else{
					Object[] obj = {"Bien joué! Vous avez gagné la partie"};
					JOptionPane.showMessageDialog(null,obj,"Fin de la partie",JOptionPane.INFORMATION_MESSAGE);
				}
			}
			else{
				Object[] obj = {"Dommage, c'est "+joueurs[gagnant].nom+"(J"+(joueurs[gagnant].numero+1)+") qui a gagné la partie"};
				JOptionPane.showMessageDialog(null,obj,"Fin de la partie",JOptionPane.INFORMATION_MESSAGE);
			}
	 ...
	 * @return le numéro de la figure i choisie
	 */
	int IAChoixTuile(Joueur j){
		...
		case 0:
		default:
			do{ // on jete une tuile au hasard sauf si l'ordi possède plus d'1 exemplaire 
		...
		case 1:
			...
			if(jeuTemp.getFirstFig(3,DESCENDANT)>=0){		//s'il y a des tuiles jetées plus de 3 fois
			...
			else{ // il n'y a que des tuiles qui ont été jété une fois ou jamais
				do{ // on jete une tuile au hasard sauf si l'ordi possède plus d'1 exemplaire 
					figChoisie = rand.nextInt(14);
					if(cpt++>14 && j.mainCache.figures[figChoisie].estLibre() == false){
						break;
		...
		case 2:
			...
			else if(jeuTemp.getFirstFig(2,DESCENDANT)>=0){	//s'il y a des tuiles jetées plus de 2 fois
	...
	void calculScore(int gagnant){
			...
			for(int j = 0; j<4; j++){
				if(i!=j){	// ce ne sont pas les mm joueurs
					if(i == gagnant){		// je suis gagnant
						...
						if(j == gagnant){	// l'autre a gagné
							if(scoreInter[i]>scoreInter[j] && joueurs[j].vent==EST){	//j'ai un score plus grand et le gagnant est EST
								scoreFinal[i]+= 4*(scoreInter[i]-scoreInter[j]);	// je reçois 4* la diff de nos score
							}
							else{
								if(joueurs[i].vent==EST || joueurs[j].vent==EST){	// si je suis EST ou que l'aute est EST
									scoreFinal[i]-= 2*scoreInter[j];					// je donne 2* son score à l'autre
								}
								else{
							...
							if(joueurs[i].vent==EST){	// si je suis EST 
								...
								scoreFinal[i]+= scoreInter[i]-scoreInter[j];	// sinon différence normale
							}
						}
					}
				}
		...
		if(gagnant == 0){
			...
			if(isBetterScore){
				...
				String message = "Bien joué! Vous êtes classé dans les high scores ("+scoreFinal[0]+" pts)";
	...
	/**
	 * Fait tourner l'image icon de deg degrès et la retoune
	 */
